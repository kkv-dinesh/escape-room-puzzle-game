// frontend/src/components/Puzzle3.js
...
const Puzzle3 = () => {
    ...
    axios.post("http://localhost:8000/game/validate", { user_id: userId, board, colors, room_id })
      .then(response => {
        if (response.data.valid) {
          ...
          setCompletionMessage("You have successfully completed the final puzzle!");
          axios.post("http://localhost:8000/game/end-timer", {
            user_id: userId, room_id, board, colors, size: board.length
          });
        } else {
  ...
  const handleRetry = () => {
    ...
    navigate("/game/puzzle3");
  };

  const handleEndGame = () => {
    navigate("/game/completion");  // redirect to your final completion screen
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: "grid", gridTemplateColumns: `repeat(${boardSize}, 40px)`,
      ...
      <Dialog open={showResultDialog} onClose={() => setShowResultDialog(false)}
        ...
        <DialogTitle sx={{ color: 'black', fontWeight: 'bold' }}>
          {solutionCorrect ? "Puzzle Complete!" : "OOPS!"}
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          {solutionCorrect ? (
            <Button onClick={handleEndGame} sx={{ backgroundColor: 'black', color: 'white' }}>Finish Game</Button>
          ) : (
      ...
      <Dialog open={showInfoDialog} onClose={() => setShowInfoDialog(false)}
        ...
        <DialogContent>
          <Typography style={{ textAlign: "center", marginBottom: "20px" }}>
            This is your final room — conquer it to complete the escape!
          </Typography>
          <Typography>
            <strong>Rules:</strong>
            <ul>
              <li>The game is played on a 9x9 chessboard.</li>
              <li>Place 9 😀 emojis, one per row, such that none attack each other.</li>
